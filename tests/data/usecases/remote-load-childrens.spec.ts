...
import { HttpClientSpy, mockChildrenModel } from '@/tests/data/mocks'

import faker from 'faker'

type SutTypes = {
  sut: RemoteLoadChildrens
  httpClientSpy: HttpClientSpy
}

const makeSut = (url: string = faker.internet.url()): SutTypes => {
  const httpClientSpy = new HttpClientSpy(HttpStatusCode.ok)
  const sut = new RemoteLoadChildrens(url, httpClientSpy)
  return {
    sut,
    httpClientSpy
  }
...
describe('RemoteLoadChildrens Usecase', () => {
  ...
  test('Should return childrens on success', async () => {
    ...
    httpClientSpy.result = {
      ...
      body: [mockChildrenModel(), mockChildrenModel()]
    }
